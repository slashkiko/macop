...
public enum CompatibilityCommand {
    public static let entries: [CompatibilityEntry] = [
        ...
        .init(command: "vault", status: "unsupported", reason: "macop does not provide a vault or cloud account backend.", alternative: "macop compatibility"),
    ]

    public static func render(format: OutputFormat) -> CommandResult {
        switch format {
        case .humanReadable:
            let header = "Supported op-compatible commands: read, run, inject, item list, item get, completion\nmacop extensions: ssh, config, doctor, compatibility\nRun \"macop compatibility --format json\" for machine-readable output.\n"
